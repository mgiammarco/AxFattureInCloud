...
public class ApiFattureInCloud
{
    ...
    private static string wsUrl { get {
            return ConfigurationManager.AppSettings["WsUrl"];
        } }
        public static HttpResponseMessage GetResponse(string metodo, string parametri = "")
        {
            string url = wsUrl + metodo;
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(url);

            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            HttpResponseMessage response = client.GetAsync(parametri).Result;
        ...
        public static HttpResponseMessage PostRequest(string metodo, StringContent body)
        {
            string url = wsUrl + metodo;
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(url);
            ...
            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            HttpResponseMessage response = client.PostAsync("",body).Result;
            if (!response.IsSuccessStatusCode)
            {
                HttpContext.Current.Response.Write("<p>[" + DateTime.Now + "] - [" + metodo + "] - [" + response.ReasonPhrase + "] - [" + response.RequestMessage + "]</p>");
            }
        ...
        public static HttpResponseMessage PutRequest(string metodo, StringContent body)
        {
            string url = ConfigurationManager.AppSettings["WsUrl"] + metodo;
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(url);

            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            HttpResponseMessage response = client.PutAsync("", body).Result;
            if (!response.IsSuccessStatusCode)
            {
                HttpContext.Current.Response.Write("<p>[" + DateTime.Now + "] - [" + metodo + "] - [" + response.ReasonPhrase + "] - [" + response.RequestMessage + "]</p>");
            }
    ...
    public static api_InfoListaResponse PostInfoAccount(StringContent body)
    {
        api_InfoListaResponse result = null;
        HttpResponseMessage response = PostRequest("info/account", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
            MyDbUtility.scriviLog(json);
            result = JsonConvert.DeserializeObject<api_InfoListaResponse>(json);
        }
        else
        {
            //HttpContext.Current.Response.Write("GetContratti: " + response.ReasonPhrase);
        }

        return result;
    }

    public static API_AnagraficaNuovoSingoloResponse PostCliente(API_AnagraficaNuovoSingoloRequest cliente) {
        API_AnagraficaNuovoSingoloResponse  result = null;
        StringContent body = cliente.body();
        HttpResponseMessage response = PostRequest("clienti/nuovo", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
            MyDbUtility.scriviLog("clienti/nuovo = "+ cliente.nome + " " +  json);
    ...
    public static API_AnagraficaModificaSingoloResponse ModificaCliente(API_AnagraficaNuovoSingoloRequest cliente)
    {
        API_AnagraficaModificaSingoloResponse result = null;
        StringContent body = cliente.body();
        HttpResponseMessage response = PostRequest("clienti/modifica", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
            MyDbUtility.scriviLog("clienti/modifica = " + cliente.nome + " " + json);
    ...
    public static API_DocNuovoResponse PostFattura(Api_DocNuovoRequest fattura)
    {
        API_DocNuovoResponse result = null;
        
        StringContent body = fattura.body();
        HttpResponseMessage response = PostRequest("fatture/nuovo", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
    ...
    public static API_AnagraficaNuovoSingoloResponse PostFornitore(API_AnagraficaNuovoSingoloRequest fornitore)
    {
        API_AnagraficaNuovoSingoloResponse result = null;
        StringContent body = fornitore.body();
        HttpResponseMessage response = PostRequest("fornitori/nuovo", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
        ...
        else
        {
            ...
            MyDbUtility.scriviLog("fornitori/nuovo errore:" + response.ReasonPhrase);
        }

        return result;
    }

    public static API_AnagraficaListaResponse PostListaClienti(API_AnagraficaListaRequest cliente) {

        API_AnagraficaListaResponse result = null;
        StringContent body = cliente.body();
        HttpResponseMessage response = PostRequest("clienti/lista", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
    ...
    public static API_FatturaListaResponse PostListaFatture(API_FattureListaRequest fattura)
    {

        API_FatturaListaResponse result = null;
        StringContent body = fattura.body();
        HttpResponseMessage response = PostRequest("fatture/lista", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
    ...
    public static API_DocInfoResponse PostInfoFatture(API_DocInfoRequest info)
    {

        API_DocInfoResponse result = null;
        StringContent body = info.body();
        HttpResponseMessage response = PostRequest("fatture/info", body);
        if (response.IsSuccessStatusCode)
        {
            var risposta = response.Content.ReadAsStringAsync();
            string json = risposta.Result;
            MyDbUtility.scriviLog(json);
            result = JsonConvert.DeserializeObject<API_DocInfoResponse>(json);
        }
        else
        {
            //HttpContext.Current.Response.Write("PostFornitore: " + response.ReasonPhrase);
    ...
    public static StringContent getBodyRequest() {
        api_generic_request json = new api_generic_request();
        var stringContent = new StringContent(JsonConvert.SerializeObject(json), Encoding.UTF8, "application/json");
        return stringContent;
    }

    public static StringContent getBodyInfo()
    {
        api_generic_request json = new api_body_info();
        var stringContent = new StringContent(JsonConvert.SerializeObject(json), Encoding.UTF8, "application/json");
        return stringContent;
    }
...
{
    public List<API_DocLight> lista_documenti { get; set; }//(Array[DocLight]),
    public int pagina_corrente { get; set; }//    (integer) : Numero della pagina restituita,
    public int numero_pagine { get; set; }//(integer): Numero di pagine totali
...
public class API_DocLight
{
    ...
    public string numero { get; set; }//(string): Numero (e serie) del documento,
...
public List<AnagraficaCliente> lista_clienti { get; set; }//(Array[AnagraficaCliente], opzionale) : Lista dei clienti(solo se richiesti i clienti),
    public List<object> lista_fornitori { get; set; }//], opzionale) : Lista dei fornitori(solo se richiesti i fornitori),
    public int pagina_corrente { get; set; }//    (integer) : Numero della pagina restituita,
    public int numero_pagine { get; set; }//(integer): Numero di pagine totali
}
...
public class AnagraficaCliente {
    ...
    public string nome { get; set; }// (string): Nome o ragione sociale del cliente,
    public string referente { get; set; }// (string): Nome referente,
    public string indirizzo_via { get; set; }// (string): Indirizzo del cliente,
    public string indirizzo_cap { get; set; }//(string) : CAP del cliente,
    public string indirizzo_citta { get; set; }//(string) : Città(comune) del cliente,
    public string indirizzo_provincia { get; set; }// (string): Provincia del cliente,
    public string indirizzo_extra { get; set; }//(string, opzionale) : Note extra sull'indirizzo,
    public string paese { get; set; }//{ get; set; }//(string) : Paese(nazionalità) del cliente,
    public string mail { get; set; }//(string): Indirizzo di posta elettronica,
    public string tel { get; set; }// (string): Recapito telefonico,
    public string fax { get; set; }// (string): Numero fax,
    public string piva { get; set; }// (string): Partita IVA,
...
{
    public string nome { get; set; } //(string) : Nome o ragione sociale del cliente,
    public string referente { get; set; } //(string, opzionale): Nome referente,
    public string indirizzo_via { get; set; } //(string, opzionale): Indirizzo del cliente,
    public string indirizzo_cap { get; set; } //(string, opzionale) : CAP del cliente,
    public string indirizzo_citta { get; set; } //(string, opzionale) : Città(comune) del cliente,
    public string indirizzo_provincia { get; set; } //(string, opzionale): Provincia del cliente,
    public string indirizzo_extra { get; set; } //(string, opzionale) : Note extra sull'indirizzo,
    public string paese { get; set; } //(string, opzionale) : Paese(nazionalità) del cliente,
    public string paese_iso { get; set; } //(string, opzionale): [Solo se "paese" non è valorizzato] Codice ISO del paese(nazionalità) del cliente(formato ISO alpha-2) in alternativa al parametro "paese",
    public string mail { get; set; } //(string, opzionale) : Indirizzo di posta elettronica,
    public string tel { get; set; } //(string, opzionale): Recapito telefonico,
    public string fax { get; set; } //(string, opzionale): Numero fax,
    public string piva { get; set; } //(string, opzionale): Partita IVA,
    public string cf { get; set; } //(string, opzionale): Codice fiscale,
    public int termini_pagamento { get; set; } //(integer, opzionale): [Solo clienti] Termini di pagamento predefiniti(giorni a partire dalla data del documento),
    public bool pagamento_fine_mese { get; set; } //(boolean, opzionale) : [Solo clienti] Indica se la scadenza del pagamento deve avvenire alla fine del mese(dopo i giorni specificati in 'termini_pagamento'),
    public string cod_iva_default { get; set; } //(number, opzionale) : [Solo clienti] Codice IVA predefinito,
    public string extra { get; set; } //(string, opzionale) : Informazioni extra sul cliente,
    public bool PA { get; set; } // (boolean, opzionale): [Solo clienti] Indica se il cliente è una pubblica amministrazione,
    public string PA_codice { get; set; } //(string, opzionale): [Solo clienti con PA=true] Codice pubblica amministrazione
}
